<template>
    <div class="reportinfo">
        <div class="report_info">
            <div class="info_head">
                <span class="head_p">{{$t('message.Photo')}}</span>
            ...
            <button class="submit" @click="submit()">{{$t('message.report')}}</button>
        </div>
...
export default {
    ...
    created(){
        let _this = this
        _this.token = decodeURIComponent(_this.$route.query.token)
        _this.language = _this.$route.query.language
        _this.clickItem = _this.$route.query.clickItem
        console.log(_this.$route.query.clickItem)
        _this.fromId = parseInt(Base64.decode(decodeURIComponent(_this.$route.query.fromId)))
        _this.fromName = Base64.decode(decodeURIComponent(_this.$route.query.fromName))
        _this.toId = Base64.decode(decodeURIComponent(this.$route.query.toId))
        _this.toName = Base64.decode(decodeURIComponent(this.$route.query.toName))
        ...
        title.set(this.$route.query.clickItem)
    },
    methods: {
        getImgBase(){
            var _this = this;
      ...
      submit(){
          this.loadShow = true
          let submitUrl = this.$util.FILESAVE
          let encodeTXT = encodeURI(this.textarea.text)
        //   let decode = decodeURIComponent(encodeTXT)
          console.log(encodeTXT)
        var param = new FormData();
        
        param.append('file',this.file);
        param.append('fromId',this.fromId);
        param.append('fromName',this.fromName);
        param.append('toId',this.toId);
        param.append('toName',this.toName);
        param.append('type',this.type);
        param.append('content',this.clickItem);
        ...
        console.log(param)
          this.$http.post(submitUrl,param,{
              ...
              if(res.data.success == true){
                  let instance = Toast({
                  message: this.$t('message.Reports'),
                ...
                setTimeout(() => {
                  ...
                  this.loadShow = false
                    let instance2 = Toast({
                        message: this.$t('message.reportLimit'),
                        iconClass: "mint-toast-icon mintui mintui-field-error",
                        className:'centerToast'
                    });
                    setTimeout(() => {
